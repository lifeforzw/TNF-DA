...
def generate_fast(
    ...
    inp_tok = build_position_ids(inp_tok)
    input_ids, attention_mask, position_ids = inp_tok["input_ids"], inp_tok["attention_mask"], inp_tok['position_ids']
    ...
    left_padding = [position_ids.size(-1)-position_ids[i,-1] for i in range(batch_size)]
    ...
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            model_out = model(
                ...
                position_ids=position_ids[:, cur_context],
            ...
            for i in range(batch_size):
                new_idx = last_non_masked[i] + left_padding[i]
                #breakpoint()
            ...
            new_inp_tok = build_position_ids(new_inp_tok)
            position_ids = new_inp_tok['position_ids']
            #breakpoint()
